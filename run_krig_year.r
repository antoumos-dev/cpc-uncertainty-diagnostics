...
mu_min <- if (length(args) >= 3) as.numeric(args[3]) else 0.1
...
base_out_dir <- file.path(project_root, "diff_figures")
...
plot_stats_simple <- function(res, year_label, out_dir, xlim, ylim,
  ...
  variants <- c(
    rel_uncert = "rel_uncert_B_median"
  )
  ...
  for (lab in names(variants)) {
    ...
    ttl <- sprintf("%s%s relative uncertainty (IQR/\u03bc), thr=%.2f",
...
plot_diff_stats <- function(res_off, res_base, year_label, out_dir, xlim, ylim,
                             cap_quant = 0.99) {
  norm <- function(lst) {
    if (!is.null(lst$rel_uncert_B_median)) {
      lst$rel_uncert_Bmed    <- lst$rel_uncert_B_median
      lst$rel_uncert_B_median <- NULL
    }
    lst
  }
  plot_diff_fields(
    annual_off        = norm(res_off$annual),
    annual_on         = norm(res_base$annual),
    seasonal_off      = lapply(res_off$seasonal,  norm),
    seasonal_on       = lapply(res_base$seasonal, norm),
    variance_ref_list = res_off$variance_ref_list,
    label             = year_label,
    threshold         = res_off$min_threshold,
    out_dir           = out_dir,
    prefix            = "DIFF",
    xlim = xlim, ylim = ylim,
    cap_quant = cap_quant
...
# Find common timestamps between conv-off and conv-on (base) files
idx_off  <- NULL
idx_base <- NULL
if (file.exists(rda_file_base)) {
  e_off  <- new.env(parent = emptyenv()); load(rda_file,      envir = e_off)
  e_base <- new.env(parent = emptyenv()); load(rda_file_base, envir = e_base)
  ts_off  <- as.numeric(e_off$timestamps)
  ts_base <- as.numeric(e_base$timestamps)
  common_ts <- intersect(ts_off, ts_base)
  if (length(common_ts) == 0) stop("No common timestamps between conv-off and base files.")
  idx_off  <- which(ts_off  %in% common_ts)
  idx_base <- which(ts_base %in% common_ts)
  message(sprintf("Timestamp alignment: OFF=%d, base=%d, common=%d",
                  length(ts_off), length(ts_base), length(common_ts)))
  rm(e_off, e_base)
}

for (thr in thresholds) {
  message("Threshold: ", thresholds)

  res_off <- compute_year_threshold_stats_simple(
    rda_file = rda_file,
    min_threshold = thr,
    mu_min = mu_min,
    rel_uncert_method = "B_median",
    subset_idx = idx_off
  )
