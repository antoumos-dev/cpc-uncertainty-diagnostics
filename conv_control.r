...
YEAR <- if (length(args) >= 1) args[1] else "24"
...
n_cores <- if (nzchar(Sys.getenv("SLURM_CPUS_PER_TASK"))) {
...
load_slot <- function(f) {
  e <- new.env(parent = emptyenv())
  load(f, envir = e)
  e$output[[8]][[2]]@data
}

mclapply_progress <- function(files, label) {
  n       <- length(files)
  bsize   <- n_cores * 10L
  batches <- split(seq_len(n), ceiling(seq_len(n) / bsize))
  out     <- vector("list", n)
  for (b in seq_along(batches)) {
    idx      <- batches[[b]]
    out[idx] <- parallel::mclapply(files[idx], load_slot, mc.cores = n_cores)
    cat(sprintf("%s: %d / %d\n", label, max(idx), n))
  }
  out
}

# # ── 1. Build all_data (conv ON) ───────────────────────────────────────────────
# all_data <- mclapply_progress(file.path(path_on, rda_files), "ON")
# names(all_data) <- format(dates, "%Y-%m-%d %H:%M")
# saveRDS(all_data, file = file.path(project_root, paste0("all_data_20", YEAR, ".rds")))
# message("Saved all_data")

# # ── 2. Build all_data_off (conv OFF) ──────────────────────────────────────────
# dates_off <- as.POSIXct(sapply(rda_files_off, parse_date_from_filename),
#                         origin = "1970-01-01", tz = "UTC")
# all_data_off <- mclapply_progress(rda_files_off, "OFF")
# names(all_data_off) <- format(dates_off, "%Y-%m-%d %H:%M")
# saveRDS(all_data_off, file = file.path(project_root, paste0("all_data_conv_off_20", YEAR, ".rds")))
# message("Saved all_data_off")

setwd(project_root)

all_data     <- readRDS(paste0("all_data_20", YEAR, ".rds"))
all_data_off <- readRDS(paste0("all_data_conv_off_20", YEAR, ".rds"))
...
result <- nearest_grid(
  rda_file       = file.path("/store_new/mch/msclim/antoumos/R/develop/CPC/data_new_project",
                             paste0("precip_transformed_results_new_20", YEAR, ".rda")),
...
result_off <- nearest_grid(
  rda_file       = file.path("/store_new/mch/msclim/antoumos/R/develop/CPC/data_new_project",
                             paste0("precip_transformed_results_conv_off_new_20", YEAR, ".rda")),
  station_coords = station_coords,
  mu_min         = 0.1
)

saveRDS(result,     file = paste0("result_conv_on_20",  YEAR, ".rds"))
saveRDS(result_off, file = paste0("result_conv_off_20", YEAR, ".rds"))
...
png(filename = paste0("topography_activations_20", YEAR, ".png"),
...
plot(1, type = "n", xlim = c(320, 900), ylim = c(-140, 450),
    xlab = "Swiss easting (km)", ylab = "Swiss northing (km)",
     main = paste0("Convective Control Activations 20", YEAR), asp = 1)
